...
impl Context {
    ...
    pub fn get_latest_ledger_info_wrapped(&self) -> anyhow::Result<LedgerInfo> {
        self.get_latest_ledger_info::<aptos_node_api_v1_types::response::BasicError>()
            .map_err(|e| e.into())
    }
    ...
    pub fn render_transactions_sequential<E: InternalError>(
        ...
        mut timestamp: u64,
    ) -> Result<Vec<aptos_node_api_v1_types::Transaction>, E> {
        if data.is_empty() {
            return Ok(vec![]);
        }
        ...
        let txns: Vec<aptos_node_api_v1_types::Transaction> = data
            .into_iter()
            .map(|t| {
